...
# 사용자당 동시 검증 수 (자동 검증). 직렬(1) — 정확도/안정성 최우선.
# 서버단 백그라운드 작업이라 시간 여유 충분, 네이버 throttle 회피가 더 중요.
PER_USER_CONCURRENCY = 1

# 요청 간 페이스 (ms). concurrency=1 + pace=500ms 로 충분한 호출 간격 확보.
# 단일 사용자 296건 ≈ (290ms + 500ms) × 296 ≈ 약 4분.
AUTO_PACE_MS = 500

# 사용자 처리 청크 크기 (메모리 보호 + 네이버 부하 분산).
# 동시에 처리하는 사용자 수를 50→20 으로 축소 → 글로벌 RPS 추가 완화.
USER_CHUNK_SIZE = 20
