...
- **상위 20위 까지만** 의미가 있다. 21위 밖은 "매칭 누락" (구 "순위권 없음") 으로 통일.
...
# ─────────────────────────────────────────────────────────────────────────────
TOP_N = 20  # 상위 N위까지만 수집. 21위 이하는 "매칭 누락" 으로 통일 (UX 문구, 내부 플래그는 out_of_range).
...
async def search_map(
    ...
    *,
    ...
    - 결과 `items` 는 광고 제외 organic **상위 20개** 만 포함. 21위 이하는
      알 수 없으며, caller 는 매칭 실패 시 "매칭 누락" 으로 표시해야 한다.
    """
