...
SITE_DESCRIPTION = (
    "타지역서비스(070 가상번호) 사장님을 위한 키워드 DNA 분석, "
    "네이버 1페이지 노출 키워드 발굴, 지역 경쟁도 분석, "
    "24시간 노출 자동 체크, 타지역 순위 자동체크(동별 시계열 추적) 5종 솔루션. "
...
SITE_ROUTES: list[Route] = [
    Route(
        ...
        description=(
            ...
            "1,875개 등록 업체와 508,854개 회선 데이터로 검증된 5종 솔루션(타지역 순위 자동체크 포함)."
        ),
        priority=1.0,
        changefreq="daily",
        category="홈",
    ...
    Route(
        path="/intro",
        title='타지역 5종 솔루션 소개 — "똑같이 등록했는데 왜 우리만 안 뜰까?"',
        description=(
            "분석 → 발굴 → 진입 → 유지 → 추적으로 이어지는 5종 통합 솔루션. "
            "키워드 DNA 파싱, 1페이지 노출 키워드 발굴, 지역별 경쟁도 분석, 노출 관리 자동 체크, 타지역 순위 자동체크(동별 시계열 추적)."
        ),
        priority=0.9,
        changefreq="weekly",
        category="솔루션 소개",
    ...
    Route(
        ...
        description=(
            "070 번호 등록 → Place ID·등록 동·상호명 자동 추출 → 매일 자동체크로 4중 검증. "
    ...
    Route(
        path="/intro/rank-tracker",
        title="타지역 순위 자동체크 솔루션 — 동별 순위를 매일 자동으로 추적",
        description=(
            "070 번호·등록동·상호·추적 키워드 엑셀 한 번으로 업로드하면 "
            "매일 자동체크로 동별 순위 데이터를 수집해 시계열 그래프로 보여드립니다. "
            "타지역 진입 후 노출 순위가 올라가는지, 경쟁사에 밀리는지 수치로 확인하세요."
        ),
        priority=0.85,
        changefreq="weekly",
        category="타지역 순위 자동체크",
    ),
    # ── About 4개 ──
    Route(
        path="/about/what-is",
